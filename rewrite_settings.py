...
new_body = """  return (
    <div className="settings-container">
      <div className="settings-sidebar">
        <button className={`settings-sidebar-btn ${['profile'].includes(openSection || '') ? 'active' : ''}`} onClick={() => setOpenSection('profile')}>
          <User size={16} /> Profile
        </button>
        <button className={`settings-sidebar-btn ${['preferences'].includes(openSection || '') ? 'active' : ''}`} onClick={() => setOpenSection('preferences')}>
        ...
        <button className={`settings-sidebar-btn ${['data'].includes(openSection || '') ? 'active' : ''}`} onClick={() => setOpenSection('data')}>
