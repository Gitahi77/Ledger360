          ...
          return (
            <div key={chama.id} style={{ padding: '1rem', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--bg-app)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                <div>
                  <h3 style={{ fontSize: '0.9rem', fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>{chama.account.name}</h3>
                  <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', margin: 0, marginTop: '0.1rem' }}>
                    Payout Position: <strong style={{ color: 'var(--text-secondary)' }}>{chama.yourPayoutPosition} of {chama.totalMembers}</strong>
                  </p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: '0.9rem', fontWeight: 700, margin: 0, fontFamily: 'Space Grotesk, sans-serif' }}>{formatKES(chama.monthlyContrib)}</p>
                  <p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', margin: 0 }}>monthly target</p>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '1rem' }}>
                <div style={{ background: 'var(--bg-subtle)', padding: '0.5rem', borderRadius: 6, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', margin: 0 }}>Next Meeting</p>
                ...
                <div style={{ background: 'var(--bg-subtle)', padding: '0.5rem', borderRadius: 6, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', margin: 0 }}>Current Balance</p>
                    <p style={{ fontSize: '0.75rem', fontWeight: 600, margin: 0 }}>{formatKES(balanceMap.get(chama.account.id) ?? 0)}</p>
                  </div>
                </div>
              </div>
