...
describe('safeToSpend', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(prisma.transaction.aggregate).mockResolvedValue({ _sum: { baseAmountMinor: 0 } } as ReturnType<typeof JSON.parse>);
    vi.mocked(prisma.transaction.findFirst).mockResolvedValue(null);
  });

  it('calculates discretionary and remaining correctly with basic inputs', async () => {
    // Expected income fallback
    vi.mocked(prisma.transaction.aggregate).mockImplementation((async (args: ReturnType<typeof JSON.parse>) => {
      if (args.where?.type === 'income') return { _sum: { baseAmountMinor: 100000 } } as ReturnType<typeof JSON.parse>;
      return { _sum: { baseAmountMinor: 0 } } as ReturnType<typeof JSON.parse>;
  ...
  it('deducts loan payments correctly', async () => {
    vi.mocked(prisma.userPreferences.findUnique).mockResolvedValue({ expectedMonthlyIncomeMinor: 100000, savingRate: 0 } as ReturnType<typeof JSON.parse>);
  ...
  it('separates envelope vs unbudgeted spend without double counting', async () => {
    ...
    vi.mocked(prisma.transaction.aggregate).mockImplementation((async (args: ReturnType<typeof JSON.parse>) => {
      // Spend in envelope
      if (args.where?.categoryId === 'cat-env') return { _sum: { baseAmountMinor: 15000 } } as ReturnType<typeof JSON.parse>;
      // Spend outside envelope
      if (args.where?.categoryId?.notIn) return { _sum: { baseAmountMinor: 10000 } } as ReturnType<typeof JSON.parse>;
      return { _sum: { baseAmountMinor: 0 } } as ReturnType<typeof JSON.parse>;
    }) as ReturnType<typeof JSON.parse>);
  ...
  it('positive rollover does NOT reduce this months Safe-to-Spend (no double-withhold)', async () => {
    ...
    vi.mocked(prisma.transaction.aggregate).mockImplementation((async (args: ReturnType<typeof JSON.parse>) => {
      ...
      if (args.where?.categoryId === 'cat-env') return { _sum: { baseAmountMinor: 10000 } } as ReturnType<typeof JSON.parse>;
      return { _sum: { baseAmountMinor: 0 } } as ReturnType<typeof JSON.parse>;
    }) as ReturnType<typeof JSON.parse>);
  ...
  it('overspending an envelope effective limit reduces this months remaining (no overstatement)', async () => {
    ...
    vi.mocked(prisma.transaction.aggregate).mockImplementation((async (args: ReturnType<typeof JSON.parse>) => {
      ...
      if (args.where?.categoryId === 'cat-env') return { _sum: { baseAmountMinor: 50000 } } as ReturnType<typeof JSON.parse>;
      return { _sum: { baseAmountMinor: 0 } } as ReturnType<typeof JSON.parse>;
    }) as ReturnType<typeof JSON.parse>);
  ...
  it('allows remainingMinor to be negative (honest negative) when massively overspending unbudgeted', async () => {
    ...
    vi.mocked(prisma.transaction.aggregate).mockImplementation((async (args: ReturnType<typeof JSON.parse>) => {
      if (args.where?.categoryId?.notIn) return { _sum: { baseAmountMinor: 120000 } } as ReturnType<typeof JSON.parse>;
      return { _sum: { baseAmountMinor: 0 } } as ReturnType<typeof JSON.parse>;
    }) as ReturnType<typeof JSON.parse>);
